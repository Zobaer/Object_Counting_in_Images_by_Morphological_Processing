...
DD = im2double(imread('detected_disc_part2.bmp'));
s = size(DD);
T=imread('disc.bmp');
SE=bwmorph(T,'remove');
B=imdilate(DD,T);
...
%H = im2double(imread('rings_hough_part2.bmp'));
...
set(hc, 'AlphaData', H);
title("Circular representatn: White-our algorithm, red-Hough Transform",Fontsize = f-5);
